...
// Simple one-way hash function (for demonstration)
// In production, use a real crypto hash like SHA-256 or bcrypt
static void one_way_hash(const char *input, char *output)
{
  unsigned long hash = 5381;
  int c;

  // DJB2 hash algorithm - simple one-way string hashing
  while ((c = *input++))
    {
      hash = ((hash << 5) + hash) + c;
    }
  ...
  // Convert to hex string (one-way representation)
...
static int authenticate_user(user_t *u, const char *password)
{
  ...
  one_way_hash(password, test_hash);

  if (strcmp(u->password_hash, test_hash) == 0)
    {
      printf("✓ Authentication successful for %s (Level %d)\n",
...
static void init_user(user_t *u, const char *username, const char *password, int level, int group)
{
  ...
  one_way_hash(password, u->password_hash);
  u->user_level = level;
...
void print_user_info(user_t *user)
{
  ...
  printf("╠══════════════════════════════════════╣\n");
  printf("║ Username:     %-20s ║\n", user->username);
  printf("║ User Level:   %-20d ║\n", user->user_level);
  printf("║ Group Number: %-20d ║\n", user->group_number);
  ...
  printf("║ Last Location:%-19s ║\n", user->last_location);
  printf("║ Connection:   %-20s ║\n",
